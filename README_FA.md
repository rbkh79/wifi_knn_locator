...
- ✅ **مکان‌یابی Outdoor**: با استفاده از دکل‌های مخابراتی (Cell Towers)
- ✅ **Cell Scanner v2 (اپدیت)**: پشتیبانی از Android 10+، استخراج دقیق‌تر اطلاعات دکل (MCC/MNC/PCI/TAC/NCI برای NR)، و تشخیص اپراتورهای ایرانی بر اساس MCC/MNC
...
- ✅ **Motion-Aware Scanning (اسکن هوشمند بر اساس حرکت)**: سرویس تشخیص حرکت از شتاب‌سنج و فیلتر کالمن برای تشخیص حالت‌های ساکن / راه رفتن و تغییر هوشمند نرخ اسکن (ساکن هر 15s، راه رفتن هر 3s، توقف ناگهانی اسکن فوری)
...
### مجوزهای اندروید مورد نیاز (مهم)
- `ACCESS_FINE_LOCATION` — برای دسترسی به نتایج `TelephonyManager.getAllCellInfo()` در Android 10+
- `ACCESS_COARSE_LOCATION` — پشتیبان نسخه‌های قدیمی‌تر
- `READ_PHONE_STATE` — برای خواندن اطلاعات اپراتور (MCC/MNC) و وضعیت سیم‌کارت

> توجه: اندروید 10+ برای بازگرداندن اطلاعات کامل دکل، علاوه بر مجوز، باید "Location Service" دستگاه روشن باشد.
...
اگر هنگام build با خطاهای Kotlin مربوط به `mcc`/`mnc` مواجه شدید، آخرین نسخه شامل اصلاح native است؛ در صورتی که باز هم خطا داشتید، لاگ کامل خطا را بررسی و ارسال کنید (مثال `adb logcat` و خروجی Gradle). ادامهٔ این README در بخش Troubleshooting درباره این موارد توضیح می‌دهد.
...
1. **اجرای اسکن**: روی دکمه "اسکن WiFi" کلیک کنید
   - اپلیکیشن به صورت خودکار اسکن Wi-Fi و Cell Towers را انجام می‌دهد
    - اگر مجوزها قبلاً داده نشده یا کاربر دائماً رد کرده است، در بخش "نتایج سیگنال‌ها" یک گزینهٔ `درخواست مجوز / اسکن` مشاهده خواهید کرد که:
      - درخواست runtime permissions (`ACCESS_FINE_LOCATION` و `READ_PHONE_STATE`) را نمایش می‌دهد
      - اگر کاربر مجوزها را "همیشه رد" (permanently denied) کرده باشد، دکمهٔ باز کردن تنظیمات اپلیکیشن را پیشنهاد می‌دهد
    - پس از اجازه دادن، اپ مجدداً اسکن را اجرا می‌کند و اطلاعات اپراتور (نام اپراتور ایرانی در صورت شناسایی) را در بخش نتایج نمایش می‌دهد.
2. **نمایش نتایج**: 
...
### نمایش اپراتور و اطلاعات دکل
- در بخش `نتایج سیگنال‌ها`، اگر اسکن سلولی موفق باشد، نام اپراتور (مثلاً `MCI (Hamrah Aval)`, `MTN Irancell`, `RighTel`) بر اساس نگاشت MCC/MNC نمایش داده می‌شود.
- اطلاعاتی که از اندروید خوانده می‌شود: `cellId`, `lac`, `tac`, `pci`, `signalStrength (dBm)`, `mcc`, `mnc`, `networkType` (LTE/NR/WCDMA/GSM).

> نکتهٔ مهم: برخی دستگاه‌ها یا سازندگان (OEM) ممکن است به‌طور کامل همهٔ فیلدها را باز نگردانند. در این حالت پیغام‌های دیباگ در `adb logcat` کمک‌کننده هستند.
...
### مشکل: اسکن سلولی (Cell) یا نمایش اپراتور کار نمی‌کند

**شرح:** در برخی دستگاه‌ها یا تنظیمات اندروید، اسکن سلولی (`TelephonyManager.getAllCellInfo()`) ممکن است اطلاعاتی برنگرداند یا مقادیر MCC/MNC نامشخص باشد. همچنین اگر مجوزهای لازم یا سرویس موقعیت غیرفعال باشد، اطلاعات سلولی بازگردانده نمی‌شود.

**راه‌حل:**
1. اطمینان حاصل کنید که `Location Service` دستگاه روشن است (Settings → Location).
2. از داخل اپ، در بخش `نتایج سیگنال‌ها` دکمهٔ `درخواست مجوز / اسکن` را فشار دهید؛ این دکمه runtime permissions (`ACCESS_FINE_LOCATION` و `READ_PHONE_STATE`) را درخواست می‌کند.
3. اگر کاربر مجوزها را `permanently denied` کرده است، هنگام زدن دکمهٔ بالا اپ پنجره‌ای نشان می‌دهد و گزینهٔ باز کردن صفحهٔ تنظیمات اپ را پیشنهاد می‌کند؛ از آن برای فعال کردن مجوزها استفاده کنید.
4. اگر پس از فعال‌سازی مجوزها هنوز اپراتور نمایش داده نمی‌شود، از `adb logcat` برای مشاهدهٔ لاگ‌های native استفاده کنید (پیام‌های `MainActivity` ما پیام‌های debug می‌نویسند):

```powershell
adb logcat -s MainActivity FlutterActivity *:S
```

5. اگر هنگام build با خطای Kotlin مربوط به `mcc` یا `mnc` مواجه شدید، از آخرین نسخهٔ کد استفاده کنید؛ ما در native اصلاحاتی اعمال کردیم که `CellIdentityNr` را با `mccString`/`mncString` ایمن‌سازی می‌کند. برای ساخت دوباره از دستورات زیر استفاده کنید:
...
6. در صورت ادامهٔ مشکل، خروجی کامل Gradle و `adb logcat` را برای بررسی ارسال کنید.
